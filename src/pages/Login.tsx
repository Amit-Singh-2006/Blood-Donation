...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      if (role === 'donor') navigate('/donor', { state: { isAvailable: isEmergencyAvailable } });
        ...
        >
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-8 space-y-6 shadow-[0_10px_25px_-5px_rgba(242,13,13,0.05),0_8px_10px_-6px_rgba(0,0,0,0.01)]">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              {/* Emergency Availability Toggle (Donor only) */}
              {role === 'donor' && (
                <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex items-center justify-between transition-all">
                  <div>
                    <h4 className="text-sm font-bold text-red-900 flex items-center gap-2">
                      <span className="material-symbols-outlined text-red-500 text-lg">medical_services</span>
                      Emergency Availability
                    </h4>
                    <p className="text-xs text-red-700/80 mt-0.5">Alert me for urgent blood requests nearby.</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={() => setIsEmergencyAvailable(!isEmergencyAvailable)}
                    />
